...
class TensorState:
    ...
    def offload(self, pin_memory=True) -> None:
        ...
        self.cpu_backup = torch.empty(
            self.tensor.size(), dtype=self.tensor.dtype, layout=self.tensor.layout, device="cpu", pin_memory=pin_memory
        )
    ...
    def create_prefetch_buffer(self) -> None:
        self.prefetch_buffer = torch.empty(
            self.cpu_backup.size(), dtype=self.cpu_backup.dtype, layout=self.cpu_backup.layout, device=self.device
        )
...
class CpuOffloadHookWithOffloadHandler(CpuOffloadSavedTensorHook):
    ...
    def __init__(
        self, offload_handler: OffloadHandler, handler_extra_kwargs={}, debug=False
...
class SynchronizedGroupOffloadHandler(OffloadHandler):
    ...
    def __init__(
        self, num_minibatch: int = 1, tensor_need_offloading_checker=tensor_need_offloading_checker, debug=False
    ) -> None:
        super().__init__()
    ...
    def offload(src_tensor: torch.Tensor, pin_memory=True):
        ...
        cpu_backup = torch.empty(
            src_tensor.size(), dtype=src_tensor.dtype, layout=src_tensor.layout, device="cpu", pin_memory=pin_memory
        )
...
#     num_offload_layers: int = 1, num_prefetch_layers: int = 1, num_offload_sync_layers: float = 1.0, debug: bool = False
# ):
#     """
...
#                 warnings.warn(
#                     "There are multiple tensors that need to apply sync, "
