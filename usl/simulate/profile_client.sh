...
run_exp() {
    ...
    sleep 3
    python experiment/client_run.py -P=$PORT -SP=$SP \
        --mbps=$MBPS --batch_size=$BS --micro_batch_size=1 \
        --pmode=pipedream_wc --model=$MODEL_NAME \
        $LORA $CLIENT_OFFLOAD_ARG --step=$STEP --save_dir=$SAVE_DIR
    sleep 3
}
...
sleep 3
python experiment/client_run.py -P=$PORT -SP=$((SP_LIST[0])) \
    --mbps=$MBPS --batch_size=$((BS + 1)) --micro_batch_size=1 \
    --pmode=pipedream_wc --model=$MODEL_NAME \
    --step=$STEP --save_dir=$SAVE_DIR -OA $LORA
echo ">>> Done!"
