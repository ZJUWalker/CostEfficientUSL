...
class Client:
    def __init__(
        ...
        nvme_handle=None,  # NEW: 传入 nvme_handle
    ):
        # ---- Model & optimizer
    ...
    def _load_optimizer(self, optimizer, prefix: str, device: str):
        # 再加载优化器状态
        for state_key, state_val in optimizer.state.items():
            for inner_key, inner_val in state_val.items():
                if torch.is_tensor(inner_val):
    ...
    def _do_pipeline(self, grad_accum_steps, micro_inputs, micro_masks, micro_labels, group_id, global_batch_idx):
        ...
        # TODO head模型参数和优化器状态卸载至SSD
        # 将头模型和优化器卸载到 SSD（NVMe）上，以释放 GPU 内存。然后尝试将头模型移动到 CPU 上
        # 如果失败了，继续进行卸载操作；如果可用，清空 GPU 缓存
        try:
            # ensure model is on GPU (or whatever) before offload is harmless; _offload_model takes .detach().cpu()
            self._offload_model(self.head_model, prefix="head")
            self._offload_optimizer(self.optimizer_head, prefix="head")
            # move head model to CPU to free GPU memory
            try:
                self.head_model.to(torch.device("cpu"))
            except Exception:
                # if moving fails, ignore but continue (offload succeeded)
                pass
            # free GPU cache
            ...
            # 若卸载失败，抛出或记录日志
            # 这里保留原样，真实环境可换成 self.logger.error(...)
            raise RuntimeError(f"Failed to offload head model/optimizer to NVMe: {e}")

        # TODO 加载tail模型参数和优化器状态到GPU
        try:
            target_tail_device = _prefer_gpu_device()
            # move tail model to target device (this allocates GPU memory)
            self.tail_model.to(target_tail_device)
            # load parameters and optimizer state from nvme_paths into tail model & optimizer
            self._load_model(self.tail_model, prefix="tail", device=str(target_tail_device))
        ...
        # TODO tail模型参数和优化器状态卸载至SSD
        try:
            # offload tail model params and optimizer state
            self._offload_optimizer(self.optimizer_tail, prefix="tail")
            self._offload_model(self.tail_model, prefix="tail")
            # move tail model back to CPU to free GPU
            try:
                self.tail_model.to(torch.device("cpu"))
            except Exception:
                pass
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
        except Exception as e:
            raise RuntimeError(f"Failed to offload tail model/optimizer to NVMe: {e}")
        # TODO 加载head模型参数和优化器状态到GPU
        try:
            target_head_device = _prefer_gpu_device()
            # move head model to GPU (alloc GPU memory)
            self.head_model.to(target_head_device)
            # load params & optimizer back from NVMe
            self._load_model(self.head_model, prefix="head", device=str(target_head_device))
        ...
        # TODO head模型优化器状态卸载至SSD
        try:
            # offload optimizer state only (parameters already offloaded earlier if needed)
            self._offload_optimizer(self.optimizer_head, prefix="head")
            # optional: you might want to offload model params too depending on your pipeline timing:
            # self._offload_model(self.head_model, prefix="head")
            # move head optimizer tensors to CPU? optimizer internals already saved; if you want to free GPU, consider moving model to CPU:
            # self.head_model.to(torch.device("cpu"))
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
        except Exception as e:
