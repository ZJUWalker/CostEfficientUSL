...
class _ServerPipelineStageBase(ABC):
    """
    Base class for pipeline stages.
    Defines or implements common methods used by the `_PipelineStage` used by
    the tracing frontend and `ServerPipelineStage` used by manual frontend.
    """
    ...
    def __init__(
        ...
        if self.group_size > self.num_stages:
            raise RuntimeError(f"Pipeline group size {self.group_size} cannot be larger than number of stages {self.num_stages}")
    ...
    def _check_chunk_id(self, chunk_id: int):
        if self.chunks is None:
            raise RuntimeError("Attempted to access chunk_id before chunks have been configured.")
        if chunk_id >= self.chunks:
            raise RuntimeError(f"Chunk id {chunk_id} is out of range [0, {self.chunks})")
    ...
    def _prepare_backward_infra(self, num_microbatches: int):
        ...
        for mb_index in range(num_microbatches):
            ...
            self.grad_recv_info[mb_index] = self._create_grad_recv_info(self.act_send_info)
    ...
    def _get_recv_ops(
        ...
        for info in recv_infos:
            ...
            peer_global_rank = peer_rank if self.group is None else dist.get_global_rank(self.group, peer_rank)  # TODO
    ...
    def set_local_bwd_input(self, next_stage_bwd_outputs: Tuple[Optional[torch.Tensor], ...], mb_index: int) -> None:
        """
        ...
        if isinstance(self.submod, DistributedDataParallel):
            if last_backward:
                ...
                self.submod.reducer.prepare_for_backward(  # type: ignore[union-attr, operator]
                    list(torch.nn.parallel.distributed._find_tensors(bwd_kwargs["stage_output"]))  # type: ignore[attr-defined]
                )
    ...
    def backward_one_chunk(
        ...
        If full_backward is False, it is optional that `dw_runner` was provided to the ServerPipelineStage at __init__ time,
        ...
        # Compute backward
        if self.is_last:
            # Last stage get grad from socket TODO
            grad_payload = self.grad_from_client.get(True)
...
class ServerPipelineStage(_ServerPipelineStageBase):
    """
    A class representing a pipeline stage in a pipeline parallelism setup.

    ServerPipelineStage assumes sequential partitioning of the model, i.e. the model is split into chunks where outputs from
    one chunk feed into inputs of the next chunk, with no skip connections.

    ServerPipelineStage performs runtime shape/dtype inference automatically by propagating the outputs from stage0 to
    stage1 and so forth, in linear order.  To bypass shape inference, pass the `input_args` and `output_args` to each
    ServerPipelineStage instance.
    ...
    def __init__(
        ...
        if self.inputs_meta is not None:
            dbg_str += f"inputs: {[inp.shape for inp in self.inputs_meta]}, " f"output: {[output.shape for output in self.get_outputs_meta()]}"
        else:
            dbg_str += " running shape-inference at runtime"

        logger.debug(dbg_str)

    @deprecated("This method is not used in USL Gpipe and USL 1F1B schedule.")
    def _shape_inference(
        ...
        if (
            ...
            assert len(args) == 0, "Can't supply input args for shape inference on non-first stage"
            objects = [None]
            logger.debug(
            ...
            dist.recv_object_list(objects, src=self.prev_rank, group=self.group, device=self.device)
            recv_args = objects[0]
    ...
    def _prepare_forward_infra(
        ...
        assert self.inputs_meta is not None, "inputs_meta should be set when initializing the stage"
        # if self.inputs_meta is None:
        #     outputs = self._shape_inference(args, kwargs)  # try to remove it TODO
        # assert self.inputs_meta is not None
        # Receive info during forward
        # TODO: create args_recv_info lazily? (same needed for ServerPipelineStage)
        for chunk_id in range(num_microbatches):
            if not self.is_first:
                ...
                self.args_recv_info[chunk_id] = tuple([_RootArgPlaceholder(i) for i in self.inputs_meta])
